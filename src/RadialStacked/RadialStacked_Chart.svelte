<script>
  ...
  let data = sampleData;
  $: width = $radialStackedChartDocs[1].value; //the width of the inner radius inversed, in pixels
  $: height = width; //the outer height of the chart, in pixels
  $: innerRadius = $radialStackedChartDocs[2].value; //the radius of the inner circle, in pixels
  $: colorRange = $radialStackedChartDocs[3].value; //the fill colors for each bar stack.  The colorRange array length  MUST match number of datasets
  $: chartScale = $radialStackedChartDocs[4].value; //the scale factor from the center
  $: sorted = $radialStackedChartDocs[5].value; //the boolean value for if the dataset is sorted
  $: varFontSize = $radialStackedChartDocs[6].value; //the font size of all text on the chart, in pixels
  $: tickColor = $radialStackedChartDocs[7].value; //the color of the inner radius ticks
  $: ringColor = $radialStackedChartDocs[8].value; //the color of the scale rings
  $: scaleColor = $radialStackedChartDocs[9].value; //the color of the scale text
  $: scaleStroke = $radialStackedChartDocs[10].value; //the color of the scale text background/stroke
  $: rectLength = $radialStackedChartDocs[11].value; //the width of the color legend key, in pixels
