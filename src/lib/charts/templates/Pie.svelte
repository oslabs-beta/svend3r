<script>
    ...
    $: width = $ChartDocs[0].value // outer width of the chart, in pixels
    $: height = width // outer height of the chart, in pixels
    $: percent = $ChartDocs[1].value // format values as percentages (true/false)
    $: fontSize = $ChartDocs[2].value; // font size of the x and y values
    $: strokeWidth = $ChartDocs[3].value; // width of stroke separating wedges
    const strokeLinejoin = 'round'; // line join style of stroke separating wedges
    $: outerRadius = Math.min(width, height) * 0.5 - 60; // outer radius of the circle, in pixels
    $: innerRadius = $ChartDocs[4].value; // inner radius of the chart, in pixels
