<script>
    ...
    $: labelPosition = $ChartDocs[5].value; // the position of the label offset from center
    $: labelRadius = (innerRadius * labelPosition + outerRadius * 0.6); // center radius of labels
    $: strokeColorWOR = $ChartDocs[6].value; //stroke color when inner radius is greater than 0
    $: strokeColorWIR = $ChartDocs[7].value; //stroke color when inner radius is 0
