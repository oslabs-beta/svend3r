<script>
  ...
  $: marginOffset = $ChartDocs.marginOffset.value // the margin top, bottom, left, right margin offset relative to the radius
  $: width = $ChartDocs.width.value; // the outer width of the chart, in pixels
  $: bandThickness = $ChartDocs.bandThickness.value; // the thickness of the color band representing each dataset
  $: fontSize = $ChartDocs.fontSize.value //the label font size relative to 1% of the width of the viewport
  $: tickStep = $ChartDocs.tickStep.value; //the chart label tick spread factor
  $: scaleFormat = $ChartDocs.scaleFormat.value; // a format specifier string for the scale ticks
  $: names = $ChartDocs.names.value; // section names
  $: colors = $ChartDocs.colors.value; // section fill colors && number of colors in fill array MUST match number of subsets in data
  $: chordOpacity = $ChartDocs.chordOpacity.value; //the opacity for the charts overall chords
  $: unselectOpacity = $ChartDocs.unselectOpacity.value; //the opacity of non-select chart elements
  $: selectOpacity = $ChartDocs.selectOpacity.value; //the opacity of select chart elements
  $: tooltipBackground = $ChartDocs.tooltipBackground.value; // background color of tooltip
  $: tooltipTextColor = $ChartDocs.tooltipTextColor.value; // text color of tooltip
